...
cFrame::cFrame()
	: m_pMtlTex(NULL), m_pVB(NULL), m_nNum(0)
{
...
void cFrame::Set()
{
	...
	ST_PNT_VERTEX * pPnt = NULL;
	m_pVB->Lock(0, 0, (LPVOID*)&pPnt, 0);
	memcpy(
		pPnt,
		&m_vecVertex[0],
		m_vecVertex.size() * sizeof(ST_PNT_VERTEX)
...
void cFrame::Render()
{
	if (m_pMtlTex)
	{
		...
		// >> IndexBuffer
		g_pD3DDevice->SetIndices(m_pVI);
		g_pD3DDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 0, m_vecVertex.size(), 0, m_vecVertex.size());


		// >> vertexBuffer
		//g_pD3DDevice->DrawPrimitive(D3DPT_TRIANGLELIST, 0, m_nNum);
	}
